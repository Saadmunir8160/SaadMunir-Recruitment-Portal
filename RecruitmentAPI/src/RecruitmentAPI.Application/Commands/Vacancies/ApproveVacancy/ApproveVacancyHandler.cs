...
public class ApproveVacancyHandler : IRequestHandler<ApproveVacancyCommand, Response<bool>>
{
    ...
    public async Task<Response<bool>> Handle(ApproveVacancyCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!VacancyApprovalAuthorization.CanActOnApprovalStep(roles, currentApproval.ApprovalStep))
            return Response<bool>.FailureResponse(
                $"Your role cannot approve step {currentApproval.ApprovalStep} ({currentApproval.ApprovalStepName}).");
        ...
        currentApproval.Comments = request.Comments;
        currentApproval.ActionDate = DateTime.UtcNow;
        await _approvalCommand.UpdateAsync(currentApproval);

        if (currentApproval.ApprovalStep == 1)
        {
            // Step 1 approved by HR Manager → create Step 2 for HR Section Head
            var step2 = new VacancyApproval
            {
                VacancyId = request.VacancyId,
                ApprovalStep = 2,
                ApprovalStepName = "HR Section Head Approval",
                Status = ApprovalStatus.Pending,
                CreatedBy = userName
            };
            await _approvalCommand.AddAsync(step2);

            return Response<bool>.SuccessResponse(true, "Step 1 approved by HR Manager. Awaiting HR Section Head review.");
        }
        else if (currentApproval.ApprovalStep == 2)
        {
            // Step 2 approved by HR Section Head → Publish the vacancy
            vacancy.PublishStatus = VacancyPublishStatus.Published;
            vacancy.PublishedDate = DateTime.UtcNow;
            ...
            if (!string.IsNullOrWhiteSpace(vacancy.CreatedByUserId))
            {
                var note = new Notification
                {
                    ...
                    Subject = $"Vacancy \"{vacancy.JobTitle}\" is published",
                    Body =
                        $"Your vacancy (requisition {vacancy.RequisitionNumber}) was approved and is now published.",
                    EntityType = nameof(Vacancy),
                    EntityId = vacancy.VacancyId,
                    TemplateCode = "VacancyPublished",
            ...
            return Response<bool>.SuccessResponse(true, "Vacancy fully approved and published.");
        }

        return Response<bool>.FailureResponse("Unknown approval step.");
    }
}
